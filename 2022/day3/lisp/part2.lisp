...
(dolist (file-name (list "example.txt" "input.txt"))
  (let ((rucksacks) (matched-priority-sum 0))
    ...
    (let ((results (make-array (/ (length rucksacks) 3) :fill-pointer 0 :adjustable t)))
      (do ((i 0 (incf i 3)))
        ((>= i (- (length rucksacks) 2)) results)
        (setf results (concatenate 'vector
                                   (intersect (list (intersect (list (elt rucksacks i) (elt rucksacks (1+ i)))) (elt rucksacks (+ i 2))))
                                   results)))
